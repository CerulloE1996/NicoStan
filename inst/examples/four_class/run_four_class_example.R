...
if (!backend %in% c("Stan", "AVX512")) stop("backend must be Stan or AVX512")
...
if (engine == "cmdstanr") {
    ...
    avx_header <- system.file("include", "BayesMVP", "stan_external_functions.hpp",
                              package = "BayesMVP")
    if (backend == "AVX512" && (!nzchar(avx_header) || !file.exists(avx_header)))
        stop("Install the BayesMVP extension package before running the AVX512 arm")
    result <- run_four_class_nicostan_smoke(
        paths = paths,
        fixture_file = fixture_file,
        stan_file = stan_file,
        math_backend = backend,
        avx_header = if (backend == "AVX512") avx_header else NULL,
        output_rds = file.path(paths$runtime_directory,
                               paste0("nicostan_", tolower(backend), "_N", N, ".rds"))
    )
}
print(result[c("math_backend", "n_nuisance", "n_params_main", "automatic_nuisance_detection")])
