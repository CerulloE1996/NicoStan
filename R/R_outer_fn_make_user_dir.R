...
.make_user_dir <- function(libname, 
             ...
             ##  Sys.getenv("R_USER") ## This is BAD on windows dont use this !! (often points e.g. to OneDrive even if OneDrive uninstalled!!)
              ...
              ##
              user_NicoStan_dir <- file.path(user_root_dir, "NicoStan")
              ##
              if (!dir.exists(user_NicoStan_dir)) {
                dir.create(user_NicoStan_dir)
              }
              ##
              cat("user_NicoStan_dir =", user_NicoStan_dir, "\n")
            
              
              
              ## -------------- USER Examples dir:
              user_examples_dir <- file.path(user_NicoStan_dir, "examples")
              cat("user_examples_dir =", user_examples_dir, "\n")
              
              system_examples_dir <- system.file("examples", package = "NicoStan")
              ...
              ## -------------- USER src dir:
              user_src_dir <- file.path(user_NicoStan_dir, "src")
              cat("user_src_dir =", user_src_dir, "\n")
              
              system_src_dir <- system.file("NicoStan/src", package = "NicoStan")
              ...
              user_stan_models_dir <- file.path(user_NicoStan_dir, "stan_models")
              cat("user_stan_models_dir =", user_stan_models_dir, "\n")
              
              system_stan_models_dir <- system.file("NicoStan/inst/stan_models/", package = "NicoStan")
